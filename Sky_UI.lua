...
local Library = {
    ...
    IsLightTheme = false,
    Scheme = {
        BackgroundColor = Color3.fromRGB(255, 255, 255), -- main white background
        MainColor = Color3.fromRGB(178, 226, 255), -- light blue (controls/off state)
        AccentColor = Color3.fromRGB(100, 170, 255), -- accent blue (active/toggled)
        OutlineColor = Color3.fromRGB(160, 200, 255), -- outline / borders
        FontColor = Color3.fromRGB(20, 20, 20), -- dark font for readability
...
-- BEGIN: Sky UI - Responsive & Readability Adjustments (appended)
-- This block adjusts main window to scale-based dimensions and improves text readability for mobile.
-- It runs after the UI is created in PlayerGui named "Sky".
spawn(function()
    ...
    for i = 1, 50 do
        if player:FindFirstChild("PlayerGui") then
            gui = player.PlayerGui:FindFirstChild("Sky")
    ...
    local function setResponsive(frame, sx, sy)
        if frame and frame:IsA("GuiObject") then
            -- prefer center anchor and responsive scale size for cross-device consistency
            pcall(function()
    ...
    -- try common container names that appear in many UI libraries
    local main = gui:FindFirstChild("Main") or gui:FindFirstChild("Edge") or gui:FindFirstChild("Window") or gui:FindFirstChild("SkyMain") or gui:FindFirstChild("SkyModal")
    if main and main:IsA("GuiObject") then
        setResponsive(main, 0.92, 0.88) -- main window ~92% width, 88% height
    else
        -- fallback: choose the largest Frame descendant and apply responsive sizing
        local largest, area = nil, 0
        for _, v in pairs(gui:GetDescendants()) do
            if v:IsA("Frame") or v:IsA("ScrollingFrame") then
                local s = v.Size
                local w = (s.X.Scale ~= 0) and s.X.Scale or 0
                local h = (s.Y.Scale ~= 0) and s.Y.Scale or 0
                local a = w * h
                if a > area then largest, area = v, a end
            end
        end
        if largest then setResponsive(largest, 0.92, 0.88) end
    end

    -- Make text readable on mobile: enable TextScaled and ensure minimum TextSize
    for _, v in pairs(gui:GetDescendants()) do
        if v:IsA("TextLabel") or v:IsA("TextButton") or v:IsA("TextBox") then
            pcall(function()
                v.TextScaled = true
                if v.TextSize and v.TextSize < 18 then v.TextSize = 18 end
                -- slightly increase RichText readability if used
                if v.RichText == nil then v.RichText = false end
            end)
        end
        -- enlarge small clickable image-buttons to be touch-friendly
        if v:IsA("ImageButton") or v:IsA("ImageLabel") then
            pcall(function()
                local sx, ox = v.Size.X.Scale, v.Size.X.Offset
                local sy, oy = v.Size.Y.Scale, v.Size.Y.Offset
                if ox and ox < 48 then ox = 48 end
                if oy and oy < 48 then oy = 48 end
                v.Size = UDim2.new(sx, ox, sy, oy)
            end)
        end
    end

    -- Optional: increase spacing/padding for Buttons and Toggle hitboxes
    for _, v in pairs(gui:GetDescendants()) do
        if v:IsA("ImageButton") or v:IsA("TextButton") then
            pcall(function()
                if v.AutoButtonColor ~= nil then v.AutoButtonColor = true end
                -- if the button has a corner UI corner, ensure minimum radius
                local corner = v:FindFirstChildWhichIsA("UICorner", true)
...
-- END: Sky UI - Responsive & Readability Adjustments
